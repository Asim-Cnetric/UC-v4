...
class WorkspaceController{
  ...
  static async getWorkspaceById(req, res){
    try{
      const workspace = await WorkspaceServices.getWorkspaceByUserId(req.params.id, req.userId);
      ...
      return res
        .status(500)
        .json({ message: "Error fetching workspace data", error: error.message });
    }
  }
  ...
  static async updateWorkspace(req, res){
    try{
      ...
      const updatedWorkspace = await WorkspaceServices.updateWorkspace(updates, req.params.id, req.userId);
      ...
      return res.status(500).json({ message: 'Error updating workspace data', error: error.message });
    }
  }
  ...
  static async deleteWorkspace(req, res){
    try{
      const deletedWorkspace = await WorkspaceServices.deleteWorkspace(req.params.id, req.userId);
      if (!deletedWorkspace) {
        return res.status(404).json({ message: 'Workspace not found' });
      }
      ...
      res.status(200).json({ msg: 'Workspace deleted successfully' });
        ...
        return res.status(500).json({ message: 'Error deleting workspace data', error: error.message });
    }
  }
}
