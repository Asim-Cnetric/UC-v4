...
class WorkspaceServices {
    ...
    static async getWorkspaceByUserId(workspace_id, user_id){
        const workspace = await Workspace.findOne({ _id: workspace_id, user_id: user_id });
        return workspace;
    }
    ...
    static async createWorkspace(payload, template_id, user_id){
        ...
        const tempateDetails = TemplateService.getTemplateByUserId(template_id, user_id);
        if(!tempateDetails){
            return;
        }
    ...
    static async updateWorkspace(payload, workspace_id, user_id){
        ...
        const updatedWorkspace = await Workspace.findByIdAndUpdate({ _id: workspace_id, user_id: user_id }, update, { new: true, runValidators: true });
        return updatedWorkspace;
    }

    //Delete Workspace
    static async deleteWorkspace(id){
        const deletedUser = await User.findByIdAndDelete({ _id: workspace_id, user_id: user_id });
        return deletedUser;
    }
}